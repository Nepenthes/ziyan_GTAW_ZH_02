...
{
  u8 RC522IDBUF [ 4 ] = { 0 };//先后存放IC卡的类型和UID(IC卡序列号)
  
	
  PcdRequest ( 0x52, RC522IDBUF );//寻卡

  PcdAnticoll ( RC522IDBUF );//防冲撞
...
void RC522MS_Thread(const void *argument){
	...
	for(;;){
		...
		if ( ( ucStatusReturn = PcdRequest ( PICC_REQALL, RC522IDBUF ) ) != MI_OK )                                    //寻卡
			ucStatusReturn = PcdRequest ( PICC_REQALL, RC522IDBUF );		                                                 //若失败再次寻卡
...
void RC522(void){
	...
	tid_RC522MS_Thread = osThreadCreate(osThread(RC522MS_Thread),NULL);
}
